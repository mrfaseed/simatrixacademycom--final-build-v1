...
function CardShape3D({ hovered, logoSrc, iconClass, iconColor = NAVY, onLogoError }) {
  ...
  useEffect(() => {
    ...
    // A page can contain dozens of cards. Create a WebGL context only for the
    // card currently being explored, instead of one context per visible card.
    if (!container || !inView || !hovered) return;
    ...
    const animate = () => {
      ...
      const dt = Math.min((frameTime - lastTime) / 1000, 0.1);
      lastTime = frameTime;
  ...
  }, [inView, hovered, logoSrc, iconClass, iconColor]);

  useEffect(() => {
    setIconTextureReady(false);
  }, [iconClass, logoSrc]);

  const showPlaceholderIcon = !hovered && !logoSrc && iconClass;
  const showPlaceholderLogo = !hovered && logoSrc;

  return (
    <div ref={mountRef} className="pointer-events-none absolute inset-0" aria-hidden="true">
      {showPlaceholderLogo && (
        <div className="absolute inset-0 grid place-items-center">
          <img src={logoSrc} alt="" className="h-28 w-28 object-contain" />
        </div>
      )}
      {showPlaceholderIcon && (
        <div className="absolute inset-0 grid place-items-center">
