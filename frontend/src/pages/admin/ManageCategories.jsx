...
export function Header({ title, onAdd, searchValue, onSearchChange, searchPlaceholder = "Search..." }) {
  return (
    <div className="mb-6 flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:flex-row md:items-center md:justify-between">
      <div>
        <p className="text-[10px] font-bold uppercase tracking-[.2em] text-accent-700">Content management</p>
        <h2 className="mt-1 font-display text-2xl font-bold tracking-tight text-slate-950">{title}</h2>
        <p className="mt-1 text-xs text-slate-500">Manage and update {title.toLowerCase()} shown across the platform.</p>
      </div>
        ...
        {onSearchChange && (
          <div className="relative">
            ...
            <input
              ...
              className="min-h-11 w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-9 pr-3 text-sm text-slate-700 outline-none placeholder:text-slate-400 focus:border-brand-400 focus:bg-white focus:ring-2 focus:ring-brand-100 sm:w-64"
            />
          </div>
        )}
...
export function RowActions({ onEdit, onDelete }) {
  return (
    <div className="flex justify-end gap-2">
      <button onClick={onEdit} aria-label="Edit" title="Edit" className="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 bg-white text-slate-500 shadow-sm transition hover:border-brand-300 hover:bg-brand-50 hover:text-brand-700">
        <i className="ti ti-edit" />
      </button>
      <button onClick={onDelete} aria-label="Delete" title="Delete" className="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 bg-white text-slate-500 shadow-sm transition hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600">
