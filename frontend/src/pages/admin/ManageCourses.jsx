...
const EMPTY = {
  ...
  level: "", tier: "classic", syllabus: "", roadmap: "", designations: "",
  quiz: sampleQuiz(), image: "", order: 0, is_active: true,
};
...
export default function ManageCourses() {
  ...
  const openEdit = (row) => {
    ...
    setForm({
      ...
      roadmap: (row.roadmap || []).join("\n"),
      designations: (row.designations || []).join("\n"),
  ...
  const save = async () => {
    ...
    if (!form.roadmap.trim()) return toast.error("Add at least one roadmap step");
    if (!form.designations.trim()) return toast.error("Add at least one career designation");
    ...
    const payload = {
      ...
      designations: form.designations.split("\n").map((s) => s.trim()).filter(Boolean),
    };
    try {
    ...
    <div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? "Edit Course" : "New Course"}
        ...
        <div className="space-y-4">
          ...
          <Field label="Learning roadmap (one step per line)">
            <textarea
          ...
          <Field label="Career designations (one role per line)">
            <textarea
              className={inputClass}
              rows={4}
